...
<div id="content-main">
  <div class="row">
	...
	?>
    <div class="col-md-12">
      <h3><?=getSystemString(309)?></h3>
    </div>
    
    <div class="col-md-12">
	    <?PHP
			$this->load->view('company/campaigns/snippets/filter_campaigns');
		?>
    </div>
...
<div class="col-md-12">
  ...
  <div class="panel white" style="padding-bottom: 50px;">
	...
	<h4 class="page-title"><?=getSystemString(131)?></h4>
	<br /> 
    <table class="table table-hover" id="campaigns_table" style="margin-bottom: 5em">
      <thead>
        <tr>
          <th class="hide">
            <?=getSystemString(149)?>
...
<script>
  $(function(){
    ...
    var dTable = $('#campaigns_table').DataTable({
      columnDefs: [
        {
      ...
      serverSide: true,
      ajax: {
        url: "<?=base_url($__controller.'/getCampaignsList')?>",
        type: "POST",
        cache: false,
        data: function(d)
        {
      ...
      drawCallback: function(settings){
        $('.dataTables_length select, .dataTables_filter input').addClass('form-control');
         $("#filter_campaigns").find(".disable-btn").remove();
        $(document).find('[data-toggle="hurkanSwitch"]').each(function(){
          $(this).hurkanSwitch({
            'on':function(r){
              alert(r);
     ...
     // filter campaigns
     $("#filter_campaigns").on("submit", function(){
	     $('#campaigns_table').DataTable().draw();
	     return false;
     });
     
     $(document).on('click',"#campaigns_table tr td .hurkanSwitch", function(){
      	ChangeStatusFor($(this), 'campaign');
    });
